      ...
      while (iter.hasNext) {
        ...
        a match {
          ...
          case buf: FileSegmentManagedBuffer =>
            ...
            assert(buf.size() < sortBuffer.ioBuf.capacity,
              s"buf length is ${buf.size()}} while capacity is ${sortBuffer.ioBuf.capacity}")
            sortBuffer.ioBuf.clear()
            sortBuffer.ioBuf.limit(buf.size().toInt)
            sortBuffer.setIoBufAddress(sortBuffer.address + offset)
            val read0 = channel.read(sortBuffer.ioBuf)
            assert(read0 == buf.size())
            offset += read0
            channel.close()
            fs.close()
        }
