...
class PruningSuite extends HiveComparisonTest with BeforeAndAfter {
  ...
  // (hr = 11 and key > 3) or (hr = 12 and key < 5)  ==> (hr = 11 or hr = 12)
  createFilterPruningTest("Partition pruning 1 - pruning with both column key and partition key",
    "SELECT key, hr FROM srcpart1 WHERE (hr = 11 and key > 3) or (hr = 12 and key < 5)",
    Seq("key", "hr"),
    Seq("key", "hr"),
    Seq(
      Seq("2008-04-08", "11"),
  ...
  // (hr = 11 and key > 10000) or key > 20000 => None
  createFilterPruningTest("Partition pruning 2 - pruning with both column key and partition key",
    "SELECT key, hr FROM srcpart1 WHERE (hr = 11 and key > 10000) or key > 20000",
    Seq("key", "hr"),
    Seq("key", "hr"),
    Seq(
      Seq("2008-04-08", "11"),
  ...
  // (key > 50 and key < 100) and (hr = 11) => (hr = 11)
  createFilterPruningTest("Partition pruning 3 - pruning with both column key and partition key",
    "SELECT key, hr FROM srcpart1 WHERE (key > 50 and key < 100) and (hr = 11)",
    Seq("key", "hr"),
    Seq("key", "hr"),
    Seq(Seq("2008-04-08", "11"),
      Seq("2008-04-09", "11")))

  // (key > 50 and key < 100 and hr = 11) or (hr = 12) => (hr = 11 or hr = 12)
  createFilterPruningTest("Partition pruning 4 - pruning with both column key and partition key 1",
    "SELECT key, hr FROM srcpart1 WHERE (key > 50 and key < 100 and hr = 11) or (hr = 12)",
    Seq("key", "hr"),
    Seq("key", "hr"),
    Seq(
      Seq("2008-04-08", "11"),
  ...
  // (key > 50 and key < 100) and (hr = 11 or hr = 12) => (hr = 11 or hr = 12)
  createFilterPruningTest("Partition pruning 5 - pruning with both column key and partition key 1",
    "SELECT key, hr FROM srcpart1 WHERE (key > 50 and key < 100) and (hr = 11 or hr = 12)",
    Seq("key", "hr"),
    Seq("key", "hr"),
    Seq(
  ...
  def createFilterPruningTest(
      testCaseName: String,
      sql: String,
      expectedOutputColumns: Seq[String],
      expectedScannedColumns: Seq[String],
      expectedPartValues: Seq[Seq[String]]): Unit = {
    test(s"$testCaseName - meta store pruning test") {
      val plan = new TestHiveQueryExecution(sql).sparkPlan
      val actualOutputColumns = plan.output.map(_.name)
      val (actualScannedColumns, actualPartValues) = plan.collect {
        case p @ HiveTableScanExec(columns, relation, _) =>
          val columnNames = columns.map(_.name)
          val partValues = if (relation.catalogTable.partitionColumnNames.nonEmpty) {
            TestHive.sessionState.metadataHive.getPartitionsByFilter(relation.catalogTable,
              p.partitionPruningPred).map(_.spec.values.toList.asJava)
          } else {
            Seq.empty
          }
          (columnNames, partValues)
      ...
      assert(actualPartitions === expectedPartitions, "Filter Partitions selected do not match")
    }
  }
}
